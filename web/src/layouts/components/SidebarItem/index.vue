...
const title = computed(() => resolveMenuTitle(props.item.meta?.title))
</script>
  ...
  <div class="sidebar-item">
    <router-link v-if="!hasChildren" v-slot="{ href, navigate, isActive, isExactActive }" custom :to="resolveRoutePath(basePath, item.path)">
      <a :href="item.meta?.link ? item.meta.link : href" :class="[isActive && 'router-link-active', isExactActive && 'router-link-exact-active']" :target="item.meta?.link ? '_blank' : '_self'" @click="navigate">
        <el-menu-item :popper-class="props.popperClass" :title="title" :index="resolveRoutePath(basePath, item.path || '')">
    ...
    <el-sub-menu v-else :popper-class="props.popperClass" :title="title" :index="settingsStore.settings.app.routeBaseOn !== 'filesystem' ? resolveRoutePath(basePath, item.path) : JSON.stringify(item)">
      <template #title>
        <el-icon v-if="item.meta?.icon" class="title-icon">
          <svg-icon :name="item.meta.icon" />
        </el-icon>
