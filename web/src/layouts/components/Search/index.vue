...
const resultList = computed(() => {
  let result = []
  const matchesTitle = (title?: string | Function) => {
    return title ? resolveTitle(title).includes(searchInput.value) : false
  }
  result = sourceList.value.filter((item) => {
    ...
    if (item.breadcrumb.some(b => matchesTitle(b))) {
      flag = true
    }
    return flag
  })
  ...
  <div id="search" :class="{ searching: isShow }" @click="isShow && eventBus.emit('global-search-toggle')">
    <div class="container">
      <div class="search-box" @click.stop>
        <el-input ref="searchInputRef" v-model="searchInput" :placeholder="$t('layout.searchPlaceholder')" clearable @keydown.esc="eventBus.emit('global-search-toggle')" @keydown.up.prevent="keyUp" @keydown.down.prevent="keyDown" @keydown.enter.prevent="keyEnter">
          <template #prefix>
            <el-icon>
