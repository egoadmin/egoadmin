...
const onSuccess: UploadProps['onSuccess'] = (res) => {
  ...
  if (uploadedUrl) {
    emit('update:url', [...(props.url as string[]), uploadedUrl])
  }
  emit('onSuccess', res)
}
...
<template>
  <div class="upload-container">
    <div v-for="(item, index) in (url as string[])" :key="index" class="images">
      ...
      <div class="mask">
        <div class="actions">
          <span :title="t('upload.preview')" @click="preview(index)">
            <el-icon>
              <svg-icon name="ep:zoom-in" />
            </el-icon>
          </span>
          <span :title="t('upload.remove')" @click="remove(index)">
            <el-icon>
              <svg-icon name="ep:delete" />
            </el-icon>
          </span>
          <span v-show="url.length > 1" :title="t('upload.moveLeft')" :class="{ disabled: index === 0 }" @click="move(index, 'left')">
            <el-icon>
              <svg-icon name="ep:back" />
            </el-icon>
          </span>
          <span v-show="url.length > 1" :title="t('upload.moveRight')" :class="{ disabled: index === url.length - 1 }" @click="move(index, 'right')">
            <el-icon>
    ...
    <div v-if="!notip" class="el-upload__tip">
      <div style="display: inline-block;">
        <el-alert :title="t('upload.imagesTip', { ext: ext.join(' / '), size, width, height, max })" type="info" show-icon :closable="false" />
      </div>
    </div>
