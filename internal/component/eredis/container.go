...
func (c *Container) Build(options ...Option) *Component {
	...
	c.logger.Infof("redis initial config: %+v", c.config)
	// 不再强制设置 StubMode，使用配置文件中的 mode 设置
	...
	case RingMode:
		if len(c.config.Addrs) == 0 {
			c.logger.Panic(`invalid "addrs" config, "addrs" has none addresses but with ring mode"`)
		}
		obj := c.buildRing()
		client = obj
		// store db
	...
	default:
		c.logger.Panic(`redis mode must be one of ("stub", "cluster", "sentinel", "ring")`)
	}
...
func (c *Container) buildRing() *redis.Ring {
	// 构建分片地址映射
	addrs := make(map[string]string)
	for i, addr := range c.config.Addrs {
		shardName := fmt.Sprintf("shard%d", i+1)
		addrs[shardName] = addr
	}
	...
	for _, incpt := range c.config.interceptors {
		ringClient.AddHook(incpt)
	}

	if err := ringClient.Ping(context.Background()).Err(); err != nil {
		switch c.config.OnFail {
		case onFailPanic:
			c.logger.Panic("start ring redis", elog.FieldErr(err))
		default:
			c.logger.Error("start ring redis", elog.FieldErr(err))
		}
	}
	return ringClient
}
